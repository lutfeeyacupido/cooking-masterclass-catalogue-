    ...
    <div class="card" :class="{ soldout: !course.available }">
        <div class="img-wrapper">
            <img :src="course.img" :alt="course.title" />
                <div v-if="!course.available" class="sold-out-banner">SOLD OUT</div>
    </div>
...
function saveToWishlist() {
  if (!props.isInWishlist) {
    const confirmed = confirm(`Would you like to add: ${props.course.title} to your wishlist?`)
    if (confirmed) {
      toggleWishlist()
      alert(`You added: ${props.course.title} to your wishlist! ❤️`)
    }
  } else {
    const confirmed = confirm(`Would you like to remove: ${props.course.title} from your wishlist?`)
    if (confirmed) {
      toggleWishlist()
...
function bookCourse() {
  const confirmed = confirm(`Would you like to book: ${props.course.title}?`)
  if (confirmed) {
    alert(`Your booking was successful: ${props.course.title} !`)
  } else {
